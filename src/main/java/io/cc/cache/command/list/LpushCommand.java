...
public class LpushCommand implements Command {
    ...
    public static final String NAME = "LPUSH";
    ...
    public Reply<?> execute(final CcCache cache, final String[] args) {
        ...
        if (args.length <= 6) {
            return new ErrorReply("ERR wrong number of arguments for 'lpush' command");
        }
        ...
        int size = cache.lpush(key, values.toArray(new String[0]));
